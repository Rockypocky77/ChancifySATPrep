SAT PRACTICE TEST 11


SECTION 1: READING & WRITING (54 Questions)


Questions 1–6: Literature Passage


The night before the piano competition, Lena practiced the same piece for the twentieth time. Her fingers traced the keys automatically, but her mind lagged behind, stumbling over what-ifs.


When she was eight, playing for relatives in a crowded living room, wrong notes had made everyone laugh. Now, at seventeen, they felt heavy, like evidence. Her teacher claimed the judges wanted "expression, not perfection," but the red circles on her sheet music suggested otherwise.


Lena stopped mid-measure and let the chord fade. She realized she could no longer tell whether she was playing the piece because she loved it or because she was afraid not to. Tomorrow, under bright lights and a silent audience, she would find out.


The passage primarily explores the narrator's experience of


A) the evolution of musical performance standards from childhood to professional competitions and how expectations change over time.


B) the psychological tension between intrinsic motivation and external validation, where artistic expression becomes entangled with self-worth measurement.


C) the stylistic differences between informal family performances and formal competitive settings, highlighting how context shapes musical interpretation.


D) the technical mastery required for advanced repertoire and the physical challenges of maintaining precision under performance pressure.


Which choice provides the best evidence for the answer to the previous question?


A) "The night before the piano competition, Lena practiced the same piece for the twentieth time."


B) "When she was eight, playing for relatives in a crowded living room, wrong notes had made everyone laugh."


C) "Her teacher claimed the judges wanted 'expression, not perfection…'"


D) "Now, at seventeen, they felt heavy, like evidence."


In the passage, the phrase "her mind lagged behind, stumbling over what-ifs" most nearly suggests that Lena


A) finds the repetitive practice monotonous and struggles to maintain engagement with familiar material.


B) experiences cognitive interference where muscle memory functions automatically while her conscious mind becomes preoccupied with potential negative outcomes.


C) is mentally rehearsing various scenarios and possible consequences rather than focusing on the present moment of performance.


D) attempts to process new musical information during execution, creating a disconnect between learning and performing.


The description of Lena's childhood performances ("wrong notes had made everyone laugh") primarily serves to


A) illustrate how family members viewed musical mistakes as endearing rather than consequential, establishing a baseline of acceptance.


B) establish a temporal contrast between an environment where errors were met with affectionate humor and her current reality where mistakes carry evaluative weight and personal significance.


C) demonstrate that early training lacked the discipline necessary for serious musical development, suggesting a missed opportunity for rigorous preparation.


D) reveal her preference for intimate, supportive performance contexts over formal competitive environments that demand perfection.


Which inference about Lena is best supported by the passage?


A) She has reached a point where she questions the sustainability of continuing piano if it remains tied to external validation rather than personal fulfillment.


B) She experiences uncertainty about whether her continued engagement with piano stems from genuine artistic passion or from fear of failure and the need to prove her worth through performance.


C) She anticipates that the judging process will be biased against her, leading her to doubt the fairness of competitive evaluation systems.


D) She regrets her instrument choice and imagines that selecting a different musical path might have led to greater satisfaction and less pressure.


The tone of the passage is best described as


A) wryly observant with moments of gentle irony that highlight the absurdity of performance anxiety.


B) frustrated and accusatory, pointing blame at the competitive system and its evaluative pressures.


C) resolute and confident, suggesting that determination will overcome obstacles and lead to success.


D) contemplative and anxious, capturing the internal conflict and psychological weight of performance expectations through reflective, measured prose.


Questions 7–12: Social Science Passage


In recent years, remote work has shifted from a rare perk to a common arrangement. Surveys show that many employees value the flexibility to choose when and where they work. Some report better focus at home, while others appreciate avoiding long commutes.


However, not all jobs are equally suited to remote work. Positions that depend on physical presence—such as nursing, manufacturing, and food service—cannot simply move online. Even for office jobs, managers worry about maintaining team cohesion and mentoring early-career employees who may feel disconnected through screens.


Rather than treating remote work as a universal solution or a temporary fad, some organizations are experimenting with "hybrid" models. In these arrangements, employees spend part of the week in the office and part working elsewhere. Early data suggest that hybrid schedules can preserve in-person collaboration while still offering flexibility.


The primary purpose of the passage is to


A) advocate for a complete transition to remote work arrangements, arguing that technological advances make physical offices obsolete and that employee preferences should drive organizational change.


B) examine the growing prevalence of remote work arrangements, identify both advantages and limitations of fully remote models, and present hybrid approaches as potential solutions that address multiple concerns simultaneously.


C) demonstrate that remote work negatively impacts productivity, collaboration, and employee development across all industries and job categories without exception.


D) encourage organizations to return to traditional office-based work models, emphasizing the superior benefits of in-person interaction and dismissing remote work as a temporary trend.


According to the passage, which factor limits the spread of remote work?


A) Workers find digital communication tools inadequate substitutes for face-to-face interaction and resist adopting screen-based collaboration methods.


B) Research consistently shows that remote work arrangements result in decreased output and efficiency compared to traditional office settings.


C) Certain occupations fundamentally depend on physical presence and direct interaction with materials, equipment, or people, making remote work structurally impossible for those roles.


D) The financial costs associated with implementing hybrid work models, including technology infrastructure and space reconfiguration, exceed what most organizations can afford.


The author distinguishes between fully remote work and hybrid models primarily to


A) demonstrate that hybrid arrangements have lower adoption rates and less employee satisfaction compared to fully remote options.


B) establish that complete remote work is fundamentally unworkable and that some form of in-person presence is always necessary for organizational success.


C) indicate that hybrid schedules represent a compromise that employees accept reluctantly when full remote work is unavailable, rather than a preferred option.


D) highlight how certain organizations are exploring solutions that combine elements of both remote and in-person work to address multiple competing needs and concerns.


Which inference is best supported by the passage?


A) Different work arrangements suit different contexts, and a one-size-fits-all solution fails to account for the varied requirements of different industries, roles, and individual preferences.


B) The shift toward remote work represents a temporary phenomenon that will reverse as organizations recognize the limitations and return to traditional office-based models.


C) Hybrid work models completely resolve issues of employee isolation and disconnection that arise in fully remote environments, providing perfect solutions to all remote work challenges.


D) Most workers find daily office commutes preferable to remote work options, valuing the routine and structure that traditional work arrangements provide.


The author's attitude toward remote and hybrid work can best be described as


A) unreservedly positive, presenting remote work as an unqualified improvement over traditional arrangements without addressing potential drawbacks.


B) fundamentally negative, treating remote work as a problematic trend that undermines workplace effectiveness and should be avoided.


C) measured and analytical, recognizing both the advantages that remote work offers to certain employees and the limitations that prevent it from being universally applicable, while exploring hybrid solutions.


D) neutral and disengaged, presenting information without taking a position or expressing interest in the implications of different work arrangements.


Which choice best describes the structure of the passage?


A) An anecdotal example from an individual's experience provides empirical evidence that validates a broader scientific principle about work arrangements.


B) A contemporary development is presented, followed by analysis of constraints and challenges it faces, concluding with exploration of alternative solutions that address multiple concerns.


C) A workplace challenge is identified and analyzed, leading to a conclusive resolution that eliminates all associated difficulties and provides a definitive answer.


D) Multiple distinct workplace phenomena are introduced sequentially without establishing connections or relationships between them.


Questions 13–20: Natural Science Passage with Data


A researcher investigated how temperature affects the rate of a simple chemical reaction. She ran the reaction at four different temperatures and measured the average reaction time (in seconds) for a visible color change to occur.


The results are described as follows:


 • At 10°C, the reaction took 40 seconds on average.


 • At 20°C, the reaction took 24 seconds on average.


 • At 30°C, the reaction took 15 seconds on average.


 • At 40°C, the reaction took 12 seconds on average.


Based on these data, the researcher concluded that increasing temperature generally decreases reaction time, though the size of the change in reaction time is not the same at every temperature increase.


The main purpose of the passage is to


A) establish that chemical reactions universally cease functioning when temperatures exceed 40 degrees Celsius, demonstrating an absolute temperature limit.


B) present experimental methodology and results that demonstrate the relationship between temperature variation and the rate at which a chemical reaction proceeds, including observations about the pattern of change.


C) analyze the differences between reaction mechanisms in liquid versus gaseous states and how phase affects temperature sensitivity.


D) demonstrate that elevated temperatures are necessary prerequisites for initiating chemical transformations and that low temperatures prevent reactions entirely.


Between which pair of temperatures does the reaction time decrease by the greatest number of seconds?


A) 10°C and 20°C


B) 20°C and 30°C


C) 10°C and 30°C


D) 30°C and 40°C


Compared with the reaction at 20°C, the reaction at 30°C


A) requires a greater duration to reach completion, indicating that higher temperatures slow reaction progress.


B) reaches completion in less time, demonstrating that increased temperature accelerates the reaction process.


C) exhibits identical completion times, suggesting that temperature changes within this range have negligible effects on reaction kinetics.


D) produces results that fall outside measurable parameters, making comparison impossible with available instrumentation.


The data most strongly support which statement?


A) Reaction duration increases in a linear, predictable manner as temperature rises, following a consistent mathematical relationship.


B) No discernible pattern connects temperature variations to reaction timing, indicating that these variables operate independently.


C) Elevated temperatures tend to correlate with reduced reaction durations, though the magnitude of change varies across different temperature intervals rather than following a uniform pattern.


D) Each 10-degree temperature increase produces exactly a 50% reduction in reaction time, creating a precise halving effect that applies consistently across all temperature ranges.


Suppose the researcher tested the reaction at 50°C. Which prediction is most reasonable based on the pattern in the data?


A) The reaction would take exactly 4 seconds.


B) The reaction time would likely be somewhat shorter than 12 seconds but not drop to zero.


C) The reaction time would return to 40 seconds.


D) The reaction would no longer occur.


Which sentence from the passage best supports the idea that the effect of temperature change is not identical at each step?


A) "A researcher investigated how temperature affects the rate of a simple chemical reaction."


B) "She ran the reaction at four different temperatures…"


C) "Based on these data, the researcher concluded that increasing temperature generally decreases reaction time, though the size of the change in reaction time is not the same at every temperature increase."


D) "The results are described as follows…"


If the data were graphed with temperature on the horizontal axis and reaction time on the vertical axis, the line of best fit would most likely have


A) an upward-sloping trend, indicating that increased temperature values correspond to increased reaction duration measurements.


B) a downward-sloping trend, showing that as temperature increases along the horizontal axis, reaction time decreases along the vertical axis.


C) a horizontal line with no slope, demonstrating that reaction time remains constant regardless of temperature variations.


D) a complex pattern with segments that slope upward and downward in alternating sequence, reflecting inconsistent temperature-time relationships.


In the context of this passage, the word "effect" in "effect of temperature" most nearly refers to


A) the visual characteristics and physical properties of the temperature-measuring instrument used in the experiment.


B) the underlying mechanism or factor that initiates and drives the chemical reaction process itself.


C) the measurable impact or influence that variations in temperature exert on the duration required for the reaction to proceed to completion.


D) the subjective interpretation or evaluative judgment that the researcher forms regarding the experimental outcomes and their significance.


Questions 21–30: Paired Passages (Humanities / Education)


Passage 1


When I take notes by hand, my pages look messy—arrows connecting ideas, comments squeezed into margins, entire paragraphs crossed out and rewritten. Typing on a laptop, by contrast, produces neat rows of text that could almost be mistaken for a finished essay.


Yet my messy notebook is where understanding happens. Writing slowly forces me to choose what matters instead of trying to capture every word the speaker says. Later, when I flip through those pages, the odd spacing and quick sketches remind me how ideas fit together. I am not just recording information; I am building a map of my thinking in real time.


Passage 2


College instructors increasingly see laptops in their classrooms, and the change is not purely positive. Some students use devices to follow along with slides, but others drift into email or social media. Studies comparing handwritten and typed notes suggest that students who type are more likely to copy lectures verbatim, while students who write by hand tend to summarize and reorganize ideas.


This does not mean laptops should be banned. Instead, educators might teach students how and when to use different note-taking tools. For complex discussions, handwritten notes may help students process ideas more deeply. For courses that require large amounts of factual detail, carefully structured digital notes can be useful. The goal is not to declare one method superior but to encourage intentional choices.


Both passages most strongly suggest that


A) digital note-taking consistently interferes with learning and should be eliminated from all educational contexts regardless of circumstances.


B) handwritten notes possess superior organizational qualities and visual clarity compared to digitally produced text, making them inherently more effective.


C) the method students employ for capturing information during lectures influences their cognitive engagement, comprehension depth, and ability to synthesize material.


D) educational institutions should implement universal bans on laptop computers in classroom settings to prevent distraction and improve learning outcomes.


The central claim of Passage 2 is that


A) digital devices represent the future of education and should completely supplant traditional notebook-based note-taking systems in academic settings.


B) different note-taking approaches serve distinct purposes and can be valuable when selected intentionally based on learning goals, course content, and cognitive objectives.


C) educators must implement disciplinary measures against students who engage with non-academic digital content during instructional time to maintain classroom focus.


D) the specific technique used for recording information has no measurable impact on student comprehension, retention, or academic performance outcomes.


Compared with the tone of Passage 2, the tone of Passage 1 is more


A) objective and removed, presenting information without emotional investment or personal perspective.


B) reflective and intimate, drawing on personal experience and emotional connection to convey insights about the note-taking process.


C) negative and antagonistic, expressing frustration and opposition toward alternative approaches or competing viewpoints.


D) specialized and academic, employing technical terminology and formal language structures typical of scholarly discourse.


Which choice provides the best evidence in Passage 2 that handwritten notes may support deeper thinking?


A) "College instructors increasingly see laptops in their classrooms…"


B) "Some students use devices to follow along with slides…"


C) "Studies comparing handwritten and typed notes suggest that students who type are more likely to copy lectures verbatim, while students who write by hand tend to summarize and reorganize ideas."


D) "This does not mean laptops should be banned."


Both passages would most likely agree that


A) laptop computers have no legitimate place in lecture settings and should be completely excluded from classroom environments.


B) effective note-taking involves active processing, selection, and organization of information rather than verbatim transcription of all spoken content.


C) digital note-taking methods produce superior learning outcomes compared to handwritten approaches across all academic contexts and subject areas.


D) educational institutions should mandate uniform note-taking techniques for all courses to ensure consistency and standardization.


In Passage 1, the author's description of "arrows connecting ideas" and "comments squeezed into margins" primarily serves to


A) acknowledge deficiencies in penmanship and calligraphic skills that detract from note quality and readability.


B) demonstrate that the author fails to maintain focus during instructional periods and engages in tangential activities.


C) exemplify how handwritten notes capture the dynamic, non-linear process of actively synthesizing, connecting, and restructuring information during learning.


D) indicate that the author lacks genuine engagement with course material and approaches learning with indifference or apathy.


In Passage 2, the phrase "the goal is not to declare one method superior" most nearly means that the author


A) actually considers one approach definitively superior but chooses not to state this conclusion explicitly.


B) considers note-taking practices irrelevant to learning outcomes and believes students can succeed without recording information.


C) advocates for intentional selection of note-taking strategies that match specific learning contexts, course requirements, and cognitive objectives rather than defaulting to habitual approaches.


D) recommends that educational institutions eliminate traditional paper-based notebooks in favor of exclusively digital documentation systems.


Which choice best describes the relationship between the two passages?


A) Passage 1 provides an individual, experiential perspective that illustrates and reinforces the broader, research-based observations and recommendations presented in Passage 2.


B) Passage 1 presents evidence or viewpoints that directly conflict with or undermine the scientific findings and conclusions reported in Passage 2.


C) Passage 2 condenses the narrative events, character development, and story progression contained within Passage 1 into a brief synopsis.


D) The passages address completely separate subjects with no thematic, conceptual, or substantive connections between them.


On which point would the authors most likely disagree?


A) Whether handwritten note-taking methods facilitate students' ability to identify relationships, patterns, and connections between different concepts and pieces of information.


B) Whether students occasionally engage in inappropriate or non-academic uses of laptop computers during instructional periods.


C) Whether educational institutions should implement complete prohibitions on laptop computers in classroom environments, eliminating all digital devices regardless of context or purpose.


D) Whether the process of condensing and synthesizing information into concise summaries represents an essential component of effective note-taking practices.


The final sentence of Passage 2 ("The goal is not… intentional choices.") mainly serves to


A) present an additional negative assessment of laptop computers and their detrimental effects on classroom learning environments.


B) emphasize that the author supports adaptable, context-aware selection of note-taking approaches rather than rigid adherence to a single method, encouraging strategic decision-making.


C) establish that digital note-taking systems consistently produce superior efficiency and effectiveness compared to all alternative approaches.


D) recommend that students maintain their existing note-taking routines without modification, avoiding experimentation with different techniques.


Questions 31–37: Writing / Expression of Ideas


(1) Many high schools are experimenting with later start times. (2) Advocates argue that teenagers' natural sleep cycles make early mornings especially difficult. (3) Research showing that students who get enough sleep have better attention, mood, and academic performance. (4) However, shifting start times can complicate bus schedules and after-school activities. (5) Some families worry that a later dismissal will conflict with part-time jobs or responsibilities at home. (6) Despite these challenges, several districts that adopted later start times have reported improved attendance and fewer tardies.


Which choice best revises sentence 3 to create a complete, grammatically correct sentence?


A) Research showing that students who get enough sleep have better attention, mood, and academic performance.


B) Research shows that students who get enough sleep have better attention, mood, and academic performance.


C) Showing that students who get enough sleep have better attention, mood, and academic performance.


D) Research, which shows that students who get enough sleep have better attention, mood, and academic performance.


The writer wants to add the following sentence to emphasize that this issue affects the broader community:


Local employers, coaches, and family members must also adjust to any changes.


Where should this sentence be placed?


A) Before sentence 1


B) After sentence 2


C) After sentence 3


D) After sentence 5


Which choice provides the most concise and appropriate version of sentence 4?


Current sentence 4:


However, shifting start times can complicate bus schedules and after-school activities.


A) However, shifting start times can sometimes, in some cases, complicate bus schedules and after-school activities.


B) However, shifting start times can complicate bus schedules and after-school activities.


C) However, shifting start times can maybe complicate bus schedules and also after-school activities and other things.


D) However, to shift start times is something that can complicate bus schedules and after-school activities.


In sentence 2, which revision best improves the precision of the phrase "especially difficult"?


A) especially annoying


B) especially inconvenient and bad


C) especially misaligned with their biology


D) especially not fun


Sentence 6 begins with the phrase "Despite these challenges." Which choice best describes the function of sentence 6 in the paragraph?


A) It contrasts the difficulties of later start times with some positive outcomes.


B) It repeats the challenges described earlier in the paragraph.


C) It introduces an unrelated topic about school lunches.


D) It undermines the idea that sleep affects academic performance.


Which revision to sentence 1 best introduces the topic with more precision?


Current sentence 1:


Many high schools are experimenting with later start times.


A) Many schools do things with their schedules.


B) Many high schools are experimenting with changes.


C) Many high schools are doing different start times and sometimes end times.


D) Many high schools are experimenting with later start times in response to concerns about teen sleep.


The writer wants to add a concluding sentence that reinforces the idea that decisions about start times should balance evidence and practical concerns. Which choice best accomplishes this goal?


A) High school is a very important time in a person's life.


B) Ultimately, communities must weigh scientific evidence about sleep against the logistical realities of their schools and families.


C) Teenagers should simply go to bed earlier so schedules do not need to change.


D) Schools should always choose the most convenient schedule for bus drivers.


Questions 38–54: Standalone Writing & Grammar


For each question, choose the best version of the underlined portion. If the sentence is best as written, choose A.


The committee of student leaders are planning a charity concert for next month.


A) are planning


B) is planning


C) have planned


D) were planning


Because the video tutorial was unclear, many students they submitted their assignments incorrectly.


A) many students they submitted


B) many students submitted


C) many students, submitting


D) many students had submitted


The instructions asked us to be polite, specific, and we should be brief in our emails.


A) polite, specific, and we should be brief


B) polite, specific, and brief


C) politely, specifically, and be brief


D) polite, specific, and briefly


If a person wants to succeed on group projects, they must communicate clearly and meet deadlines.


A) they must


B) he or she must


C) one musts


D) you musts


Researchers identified a new species, it lives in deep ocean vents.


A) it lives


B) which lives


C) it, living


D) and it lives


Having reviewed the survey results, the recommendation was to expand the tutoring program.


A) the recommendation was to expand


B) the committee recommended expanding


C) expanding the tutoring program was recommended by them


D) there was a recommendation made to expand


Sales increased by 15 percent this quarter. Nevertheless, the company also reduced advertising costs.


A) Nevertheless


B) Meanwhile


C) Consequently


D) Furthermore


The presentation was not only informative but it was also engaging for the audience.


A) but it was also engaging for the audience.


B) but also engaging for the audience.


C) and it was also engaging for the audience too.


D) but it also was engaging the audience.


Over the past decade, the city's population has been grown by nearly 20 percent.


A) has been grown


B) has grown


C) had grew


D) was growing


Many students use social media to share artwork, they also receive feedback from peers.


A) they also receive feedback


B) also receiving feedback


C) and also they receive feedback


D) and they also receive feedback


Walking through the art gallery, the bright colors of the paintings overwhelmed me.


A) the bright colors of the paintings overwhelmed me.


B) I was overwhelmed by the bright colors of the paintings.


C) there were bright colors from the paintings that overwhelmed me.


D) it was the bright colors of the paintings that overwhelmed me.


Between you and I, this schedule is unrealistic.


A) I


B) me


C) myself


D) we


The board of directors have approved the new sustainability plan.


A) have approved


B) has approved


C) are approving


D) were approving


Recent evidence from multiple studies support the idea that regular exercise improves sleep quality.


A) support


B) supports


C) are supporting


D) have been supporting


Many ecosystems rely on keystone species. For instance, sea otters help control sea urchin populations.


A) Nevertheless


B) For instance


C) Conversely


D) Instead


Despite showing early promise. The program failed to meet its long-term goals.


A) NO CHANGE


B) promise, the program


C) promise; the program


D) promise, and the program


After analyzing the data carefully, recommendations were made by the researchers to revise the policy.


A) recommendations were made by the researchers


B) the researchers made recommendations


C) the researchers' recommendations were made


D) the making of recommendations was done by the researchers


SECTION 2: MATH (Questions 55–98)


Algebra — Questions 55–64


A company's production cost C (in thousands) is modeled by C = 7(x + 2), where x is units produced. The company's revenue R is modeled by R = 3(x - 4) + k, where k is a fixed base revenue. If the company breaks even when C = R and k = 50, what is the value of x?

A) 3
B) 4
C) 6
D) 7

A linear function f(x) = mx + b passes through the points (1, -2) and (5, 6). What is the value of m (the slope)?

A) 1/2
B) 3/2
C) 2
D) 4

In a chemistry experiment, the concentration must be at most 13 units. If concentration = 4x - 3, where x is the amount added, what is the largest integer value of x?

A) 3
B) 4
C) 5
D) 6

A streaming service charges a monthly fee of $12 plus $2 for each movie rented. If a customer's bill for one month is $32, how many movies did the customer rent?

A) 8
B) 10
C) 12
D) 16

A linear function f(x) = -3x + 9 has an x-intercept. What is the x-coordinate of this intercept?

A) -3
B) -9
C) 3
D) 9

A system of equations is given: 2x + y = 11 and x - y = 1. If (a, b) is the solution to this system, and a third equation cx + dy = 25 also passes through (a, b) where c = a and d = b, what is the value of c + d?

A) 2
B) 3
C) 5
D) 4

A gym charges a one-time sign-up fee of $50 and a monthly fee of $15. Which equation represents the total cost C for m months?

A) C = 50m + 15
B) C = 15m + 50
C) C = 50 + m + 15
D) C = 65m

The function f(x) = 5x - 7 models a quantity over time. What does the point (0, -7) represent in this context?

A) The slope of the function
B) The average rate of change
C) The starting value when x = 0
D) The maximum value of the function


Solve the equation |x - 2| = 5.


A) x = -3 or x = 7


B) x = 3 or x = 5


C) x = -7 or x = 3


D) x = -5 or x = 2


The inequality 3x + 2 > 11 is equivalent to


A) x > 3


B) x > 11/3


C) x < 3


D) x ≤ 3


Advanced Math — Questions 65–76


The quadratic function f(x) = x² - 6x + 5 has its vertex at


A) (2, -3)


B) (3, -4)


C) (3, 4)


D) (6, 5)


Let g(x) = 2x². What is the value of g(3)?


A) 6


B) 9


C) 18


D) 36


Solve the equation:


 x² - 4x - 12 = 0


A) x = -6 only


B) x = 2 only


C) x = -2 or x = 6


D) x = 2 or x = 4


The expression (x - 3)(x - 5) is equivalent to


A) x² - 8x + 15


B) x² + 8x + 15


C) x² - 15x + 8


D) x² + 2x - 15


A savings account is modeled by the function A(t) = 200(1.05)ᵗ, where t is the number of years after the account is opened. What does the factor 1.05 represent?


A) A 5% decrease in the account each year


B) A 50% increase in the account each year


C) A 5% increase in the account each year


D) A 0.5% increase in the account each year


The function f(x) = 1/(x - 4) is defined for all real numbers except x = 4. What is the domain of f(x)?


A) All real numbers


B) All real numbers except x = 4


C) All real numbers except x = -4


D) x > 4 only


The table below defines a linear function f:


x | f(x)


 0 | 5


 1 | 8


 2 | 11


If the pattern continues, what is f(4)?


A) 13


B) 14


C) 15


D) 17


(Student-Produced Response)


 Solve for x:


 5/x = 2/3


 Enter your answer as a positive fraction or decimal.


The equation of a parabola is given by


 y = (x + 2)² - 7.


Which transformation describes the graph of this parabola relative to the graph of y = x²?


A) Shifted 2 units right and 7 units up


B) Shifted 2 units left and 7 units down


C) Shifted 2 units left and 7 units up


D) Shifted 2 units right and 7 units down


(Student-Produced Response)


 Solve for x:


 9x² = 81


 Enter the positive solution.


Let f(x) = 3x - 1 and g(x) = x + 4. What is the value of g(f(2))?


A) 5


B) 7


C) 9


D) 11


(Student-Produced Response)


 If k(x) = (x² - 1)/(x - 1) for x ≠ 1, what is k(4)?


Problem Solving & Data Analysis — Questions 77–88


In a survey of 300 people, 45% preferred Product A. How many people preferred Product A?


A) 90


B) 120


C) 135


D) 150


A jacket originally costs $80 and is on sale for 30% off. What is the sale price?


A) $24


B) $32


C) $40


D) $56


(Student-Produced Response)


 A mixture contains ingredients in a ratio of 5:3 (ingredient X : ingredient Y). If there are 20 units of ingredient X, how many units of ingredient Y are in the mixture?


A ticket seller records the number of tickets sold each day:


Monday: 10


 Tuesday: 14


 Wednesday: 18


 Thursday: 20


 Friday: 28


What is the mean number of tickets sold per day?


A) 14


B) 16


C) 18


D) 20


A set of 4 test scores has a mean (average) of 72. The first three scores are 70, 68, and 76. What is the fourth score?


A) 70


B) 74


C) 78


D) 80


A bag contains 4 red marbles, 5 blue marbles, and 1 green marble. If one marble is chosen at random, what is the probability that it is blue?


A) 1/4


B) 2/5


C) 1/2


D) 3/5


(Student-Produced Response)


 The price of a concert ticket decreases from $250 to $200. What is the percent decrease?


 (Enter your answer as a whole number.)


A scatterplot of hours of screen time per day (horizontal axis) versus hours of sleep per night (vertical axis) shows a strong negative linear relationship. Which conclusion is most reasonable?


A) Students who sleep more tend to use more screens.


B) Students who spend more time on screens tend to sleep fewer hours.


C) Screen time and sleep are unrelated.


D) Screen time always causes insomnia in all students.


In a group of students, 40 like soccer, 25 like basketball, and 15 like both sports. How many students like at least one of the two sports?


A) 40


B) 50


C) 55


D) 65


Data set A and data set B have the same mean. Data set A's values are all very close to the mean, while data set B's values are spread out far above and below the mean. Which statement is most accurate?


A) Data set A has a larger standard deviation than data set B.


B) Data set B has a larger standard deviation than data set A.


C) Both sets have the same standard deviation.


D) Neither set has a standard deviation.


(Student-Produced Response)


 A bank account earns simple interest at a rate of 4% per year on an initial balance of $1,500. How much total interest is earned after 5 years?


A linear model for a data set is y = -3x + 40. When x increases by 1, what happens to y?


A) It increases by 1.


B) It increases by 3.


C) It decreases by 3.


D) It decreases by 40.


Geometry & Trigonometry — Questions 89–98


A right triangle has legs of lengths 5 and 12. What is the length of the hypotenuse?


A) 10


B) 13


C) 14


D) 17


A rectangle has an area of 63 square units and a width of 7 units. What is its length?


A) 6


B) 7


C) 9


D) 10


A circle has radius 10 units. What is its circumference, in terms of π?


A) 10π


B) 20π


C) 100π


D) 200π


(Student-Produced Response)


 A cylinder has a radius of 4 units and a height of 9 units. Its volume is given by V = πr²h.


 What is the volume of the cylinder in terms of π?


Two points in the coordinate plane are (1, 2) and (7, 2). What is the distance between the points?


A) 6


B) √10


C) √34


D) 8


Triangle ABC is similar to triangle DEF. The sides of triangle ABC are 3, 4, and 6. The corresponding sides in triangle DEF are 9, 12, and 18. If the perimeter of triangle ABC is 13, what is the perimeter of triangle DEF?


A) 26


B) 32


C) 39


D) 52


In a right triangle, cos(θ) = 4/5. What is sin(θ)?


A) 4/5


B) 3/5


C) 5/3


D) 5/4


(Student-Produced Response)


 Convert (π/6) radians to degrees.


A regular pentagon has side length 7 units. What is its perimeter?


A) 21


B) 35


C) 42


D) 49


(Student-Produced Response)


 In a 30°–60°–90° right triangle, the hypotenuse has length 10. What is the length of the side opposite the 30° angle?


Test 11 Answer Key


Reading & Writing (1–54)


B


D


C


B


B


D


B


C


D


A


C


B


B


A


B


C


B


C


C


B


C


B


C


C


B


B


C


A


C


B


B


D


B


B


B


B


B


B


B


B


D


B


B


B


B


B


B


B


Math (55–98)
55. A
56. C
57. B
58. B
59. C
60. D
61. B
62. C
63. A
64. A
65. B
66. C
67. C
68. A
69. C
70. B
71. D
72. C
73. B
74. C
75. C
76. D
77. C
78. D
79. C
80. C
81. B
82. C
83. B
84. B
85. B
86. B
87. C
88. B
89. A
90. B
91. B
92. C
93. A
94. B
95. B
96. B
97. B
98. D


